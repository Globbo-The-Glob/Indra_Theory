...
class Net:
    def __init__(self,name,Adj,n,k,dt,var,mean,peaks_phase,means_phase,peaks_freq,means_freq):
        ...
        for kk in range(0,peaks_phase):
            # distribute a proportion of clocks about mean
            a = 0 + nn*i
            b = nn + nn*i
            self.state[a:b] = np.random.normal(means_phase[i],0.01,(nn,1))
            i += 1
    ...
    def Update(self,query_nat, query_update,T,res):
        ...
        for i in range(self.n):
            ...
            for j in range(self.n):
                ...
                if query_nat == True:
                    ...
                    #     if i != j:    
                    #         self.Adj[i,j] = (T/res)*np.cos(self.state[j]-self.state[i])# update coupling
            self.I[i] = I_n
            self.Iw[i] = I_f #check change!
        # print(np.shape(self.Iw)) 
        #print(self.I)
       # print(self.Iw)
        #print('pre state = {}'.format(self.state))
        #+ self.I #*1/self.n #+ self.noise_A*np.random.randn(self.n,1)
        ...
        # if query_nat == False:
        #     self.state += self.nats + self.coupling*self.I
